if [ "$1" = "start" ] ; then
  ...
  nohup ./audit-access.sh 1>audit-access.log &>audit-access.log &
  nohup ./audit-cmis.sh 1>audit-cmis.log &>audit-cmis.log &
elif [ "$1" = "stop" ] ; then
  ...
  ps -ef | grep "/bin/sh ./audit-access.sh" | grep -v grep | awk '{print $2}' | xargs -I {} kill -9 {}
  ps -ef | grep "/bin/sh ./audit-cmis.sh" | grep -v grep | awk '{print $2}' | xargs -I {} kill -9 {}
else
